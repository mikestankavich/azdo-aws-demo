...
output "azdo_project_bootstrap_id" {
  description = "Azure DevOps bootstrap project ID"
  value       = azuredevops_project.bootstrap.id
}

output "azdo_project_bootstrap_url" {
  description = "Azure DevOps bootstrap project URL"
  value       = "${var.azdo_org_service_url}/${azuredevops_project.bootstrap.name}"
}
...
# =============================================================================
# Bootstrap Pipeline Information
# =============================================================================

output "bootstrap_pipeline_name" {
  description = "Name of the bootstrap pipeline to run next"
  value       = azuredevops_build_definition.bootstrap_pipeline.name
}

output "bootstrap_pipeline_url" {
  description = "URL to the bootstrap pipeline"
  value       = "${var.azdo_org_service_url}/${azuredevops_project.bootstrap.name}/_build?definitionId=${azuredevops_build_definition.bootstrap_pipeline.id}"
}
...
output "next_steps" {
  ...
  value = {
    step_1 = "Clone repositories: ${azuredevops_project.bootstrap.name} and ${azuredevops_project.demo.name}"
    step_2 = "Add Terraform code and pipeline YAML to bootstrap repository"
    step_3 = "Configure AWS service connections in Azure DevOps using the provided role ARNs"
    step_4 = "Run the '${azuredevops_build_definition.bootstrap_pipeline.name}' pipeline"
